...
/*
 * h_motor_acm.h - ACM 伺服电机子类（h_motor_base 的具体实现）。
 *
 * 继承 h_motor_base，把 24 个虚方法落到"通过 UART 发厂家私有 ASCII 命令、
 * 阻塞读响应、按需回调"这套实现上。
 *
 * 关键设计：
...
struct h_motor_acm {
	struct h_motor_base base;        /* 必须在第一字段             */
	...
	struct h_motor_acm_rx_buf rx_buf;
 ...
 * 老固件兼容性。 */
void h_motor_acm_init(struct h_motor_acm *me,
                      struct h_motor_acm_init_param *init_param);
