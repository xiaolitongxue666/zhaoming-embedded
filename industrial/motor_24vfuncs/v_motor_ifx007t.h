...
/**
  ******************************************************************************
  * @file    v_motor_ifx007t.h
  * @brief   IFX007T H 桥子类（垂直直流电机）—— 接 v_motor_base 的 PWM 调速
  *          实现。两路 PWM 分别驱动 H 桥两个高边臂，加 sleep1/sleep2 两根
  *          INH 引脚控制 IC 上电/休眠，再加上下两个减速区限位开关 +
  *          一路 IS 引脚电流采样。
  *
  *          继承关系：
  *              v_motor_ifx007t_t.base -> v_motor_base_t
  *
  *          ops 派发：
 ...
 * adc_channel_base_t 指针传进来即可。 */
struct adc_channel_base;
typedef struct adc_channel_base adc_channel_base_t;
...
typedef struct
{
    ...
    adc_channel_base_t *is_adc_channel;
} v_motor_ifx007t_init_param_t;
