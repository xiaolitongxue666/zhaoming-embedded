...
ch20 § 20.3 正文里"几行 GPIO 操作"是教学描述用的最小内核，目的是把 OOP 抽象骨架（3 ops + 2 回调）讲清楚就够了。仓库里这一份 `v_motor_ifx007t.c` 是产品级版本，多出来的软启动 / 加减速 / 限位 / 电流故障保护这些工程细节并没有改变 OOP 骨架，子类对外仍然只暴露 motor_stop / motor_move / fault_clear 三个 ops 函数指针，应用层调用代码一行不动。两份代码的关系跟 ch19 主控板 LED 教学版 vs 工业版的关系一样：教学版讲清楚抽象骨架，工业版把骨架在产品级现实里跑通。

## 风格约定

全仓统一 struct 风格：基类 / 子类 / ops 表都用 `struct led_base` / `struct h_motor_base` 这种带 `struct` 前缀的写法（Linux 内核风格），而不是带 `_t` 后缀的 typedef 别名风格。回调函数指针 typedef（`status_cb_t` / `cur_pos_cb_t` / `err_cb_t` / `v_motor_err_cb_t` / `v_motor_move_status_changed`）按 C 工程标准保留 typedef，这样上层注册函数签名干净。

## 关于 h_motor_acm.c

`h_motor_acm.c` 完整 1500 多行，包含：
