...
/*
 * h_motor_base.h - 水平电机基类（伺服电机抽象）。
 *
 * 24 个虚方法 + 3 个回调，覆盖工业伺服电机的全部能力域。这是全书最大的
 * ops 表。每个虚方法都对应一个真实工程问题（详见 ch20 § 20.1.1 的"24 个
 * 虚方法是怎么长出来的"）。一对照 v_motor_base 的 3 个虚方法，能直观感
 * 受到"接口规模严格匹配硬件能力": 伺服电机闭环、可编程、有 I²t 保护，
 * 所以 24 个方法都用得上；直流电机能力域窄，3 个方法就够了。
 *
 * 应用层只调本头文件下半段声明的 h_motor_base_xxx 封装函数，不直接访问
 * ops 字段。基类封装函数内部走 me->ops->xxx(me, ...) 派发到具体子类
 * （h_motor_acm 等）。
 */
 ...
 *
 *   三种运动模式（持续 / 相对 / 绝对）
 *     move_velocity_mode / move_relative_mode / move_absolute_mode
 *
 *   运动参数动态调节
...
struct h_motor_base_ops {
	...
	void (*move_velocity_mode)(struct h_motor_base *me,
	                           enum h_motor_base_move_dir dir);
 ...
 *
 * callback typedef 是 C 工程标准做法（Linux 内核 / RT-Thread / Zephyr
 * 同款）：函数指针类型不是数据 struct，typedef 成简短名字让上层注册
 * 函数签名干净。 */
typedef void (*status_cb_t)(struct h_motor_base_status *status,
                            struct h_motor_base *me);

typedef void (*cur_pos_cb_t)(double pos_mm,
                             struct h_motor_base *me);

typedef void (*err_cb_t)(enum h_motor_base_err_id err_id,
                         struct h_motor_base *me);

/* 基类对象。子类把 struct h_motor_base 放在第一字段（参考
 * struct h_motor_acm.base），向上转型零代价。
 * ops 字段加 const，与 v_motor_base 对齐: 子类的静态 ops 表
 * 落到 .rodata 段全程序共享，构造期挂上之后基类层不再改写。 */
struct h_motor_base {
	const struct h_motor_base_ops *ops;
