...
第 1 章到第 3 章做了不少事。挂号单（struct）、me 指针、字段藏起来、命名前缀、init/deinit 生命周期。但翻开你的 `led.c` 看头几行，可能还在躺着这种东西：
...
`static` 这个修饰词让编译器把这两个变量的符号写成 file-local（第 2 章 2.6.5 节讲过链接器视角）。别的文件 `extern` 都找不到。
...
费曼讲过一句话：好的设计不靠人小心，靠机制上不让错误发生。把数据按所有权归位，是机制层面的防御。第 2 章把字段藏起来防外部乱写、第 4 章把全局变量按所有权归位防内部乱写，两条加起来是数据层面的"封装"完整版。
...
数量上限固定但具体哪些槽用、什么时候用不固定时用对象池。Linux 内核的 `kmem_cache` / slab 分配器是同一个思路的工业级形态（第 17 章展开）。本章配套代码用这种写法是为了把"模块共享数据 + 实例数据"凑齐演示。
...
工业代码里这种 NULL 检查是强制的，几乎没有"分配失败就忽略"的情况。第 17 章讲 Linux 内核的 `kmalloc` 也是同样的纪律。
...
| 修饰位置 | 效果 | 例子 |
|---|---|---|
| 函数前 | file-private 函数 | `static void update_hardware(...)` |
| 文件作用域变量前 | file-private 变量 | `static int s_debug_flag` |
...
2. **异常退出走 `goto cleanup`**。Linux 内核风格，几十年实践能压住所有 leak。形态在 ch02 2.6.8 节贴过
...
以 STM32H7 系列（Cortex-M7，是 Simon 工业控制板项目用的同一代核）为例。这颗芯片的内存布局：
...
**接口型全局指针句柄**：例如 `extern struct led_base *green_led;`、`extern struct uart *console;`、`extern struct rtc_dev *system_rtc;`。这种是合理的。指针指向的对象关在驱动的 `.c` 文件里（参考 ch01 1.10 节的不透明指针），应用层只能通过 `led_on(green_led)` 这种封装函数操作它，拿不到"裸字段读写"的能力。这是对一个**单例对象的句柄绑定**，不是裸数据共享。

工业代码里 `extern xxx_base_t *xxx_dev;` 这种声明是大量存在的，每个外设设备实例（LED、按键、UART、RTC、温度传感器）通常都对应一两个全局句柄。这就是"设备树思路"在 C 里的落地：全局可见的设备名 + 不透明的设备实例。第 19 章工业实战会展开。
...
2. **应用层通过句柄访问对象字段**：写 `green_led->pin = 5` 这种应该编译报错（不透明指针保护），写 `g_temp = 100` 这种应该 code review 打回。

## 4.8 你现在的代码在 STM32 上长什么样

PC 模拟版是 `printf` 假装操作 GPIO。STM32 真实硬件上长这样（节选自 [`oop-in-c/code/04-data-classification/stm32-snippet/led_stm32.c`](https://github.com/ZhaoChengBo/zhaoming-embedded/tree/master/oop-in-c/code/04-data-classification/stm32-snippet/led_stm32.c)）：
...
嵌入式 Linux 端长这样（节选自 [`oop-in-c/code/04-data-classification/linux-snippet/led_linux.c`](https://github.com/ZhaoChengBo/zhaoming-embedded/tree/master/oop-in-c/code/04-data-classification/linux-snippet/led_linux.c)）：
...
`led_pool[8]` 在 Linux 进程的 `.bss` 段，进程启动时由 loader 清零。这个例子里整个进程生命周期不需要任何 `malloc`，但这是配套代码的演示选择，不是 Linux 编程纪律。

同样，本节的 platform 是函数式包装的教学简化版，第 11 章后演化为 ops 表。
...
封装篇到这里：第 1 章 struct + me、第 2 章 static + 不透明指针、第 3 章前缀 + init/deinit、第 4 章数据三级归位（实例 / 模块 / 常量）。所有零件凑齐了。
