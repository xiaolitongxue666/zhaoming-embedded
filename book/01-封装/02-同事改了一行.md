...
> 完全机制锁的方案在 C 里也存在，叫**不透明指针**：`.h` 里只写 `struct led;` 前向声明，字段藏到 `.c` 里，编译器直接拒绝外部读字段。代价是字段不可见后无法栈分配也无法被嵌套继承。这条路适合**跨二进制库的 ABI 边界**（`FILE *` / `pthread_t` / `sqlite3 *` / `CURL *` 这一类），在自家应用代码里几乎不用。详见 2.6.3 节。
...
这本书第 16 章会展开 `struct file` 和 `struct file_operations` 的工业级用法。这一章你只需要看到：你刚才在 `struct led` 上做的事，字段公开 + 命名纪律 + 内部工具加 `static`，Linux 内核 4000 万行代码也在做，规模不一样而已。
...
Linux 内核 kerneldoc 注释里用 `@private:` 标签写这件事。GTK GObject 的字段会写 `<private>` HTML tag。哪种风格不重要，重要的是字段第一行注释里就告诉读者「别直接写」。
...
链接器扫所有 `.o` 的全局符号表都找不到 `update_hardware`：它在 `led.c` 里加了 `static`，是 file-private 符号，外部连名字都看不到。这是机制层的硬锁。
