...
>
> 工业代码里 `pin` 是一个**编码值**，把 port + 引脚号塞进一个整数：
>
> ```c
> #define PIN(port, num)   (((port) << 4) | ((num) & 0xF))
> ```
>
> 调用方写 `PIN(D, 12)` 表示 PD12，port 信息藏在 pin 编码里。这样接口签名永远只有一个 `pin` 参数。Linux 内核 `gpio_set_value(unsigned int gpio, ...)` 是这套做法，工业控制板的 `platform_pin_write(int32_t pin, int32_t val)` 也是。第 15 章和附录 B 会把这条工业纪律展开。
...
PC 模拟版是 `printf` 假装操作 GPIO。STM32 真实硬件上长这样（节选自 [`oop-in-c/code/01-three-leds/stm32-snippet/led_stm32.c`](https://github.com/ZhaoChengBo/zhaoming-embedded/tree/master/oop-in-c/code/01-three-leds/stm32-snippet/led_stm32.c)）：
...
第 11 章一步步推出 vptr 之后，会把 platform 层从函数式重构成 ops 表式，和工业代码对齐。

## 1.9 你现在的 LED 在 Linux 用户态长什么样

嵌入式 Linux 端长这样（节选自 [`oop-in-c/code/01-three-leds/linux-snippet/led_linux.c`](https://github.com/ZhaoChengBo/zhaoming-embedded/tree/master/oop-in-c/code/01-three-leds/linux-snippet/led_linux.c)）：
...
Linux 把每个 GPIO 暴露成一个文件 `/sys/class/gpio/gpio13/value`。写 `'1'` 拉高，写 `'0'` 拉低。这种"把硬件当文件"的设计叫 sysfs，是 Linux 内核统一暴露驱动状态的标准接口（更现代的方案叫 libgpiod，附录 C 会展开）。

注意：`led.h`、`led.c`、`main.c` 还是一字不改。这次变化的胶水是把 GPIO 操作转成 open / write / close。

如果你是嵌入式 Linux 工程师，看到这个会发现：你已经在写这个抽象了。所有 Linux 驱动的用户态接口都是这个套路，从最基础的 GPIO，到复杂的网卡驱动。第 16 章会讲 Linux 内核态怎么把 sysfs 文件接到底层 GPIO controller。

同 1.8 节，这里 Linux 端的 platform 层也是函数式包装的教学简化版。真正工业级的 platform 抽象用 ops 表（虚函数表），等到第 11 章演化出 vptr 之后会把它改成和 1.10 节工业代码一致的形态。
...
反过来看，1.8 / 1.9 节给的 STM32 / Linux platform 层用的是函数式（几个 `platform_gpio_*` 函数独立 export），是教学简化版。工业代码的 platform 层和这里 `led_base` 一样，都是 ops 表（虚函数表）形式。两者都对，只是抽象程度不同。书里第 11 章之后会把 platform 层从函数式升级成 ops 表式，完成这个对齐。
...
`led_on` 这个函数在 `led.c` 里只写了一次。但屏幕上 Pin13、Pin14、Pin15 都被点亮了。因为传入的 `me` 指针不同。

完整输出和源码见 [`oop-in-c/code/01-three-leds/`](https://github.com/ZhaoChengBo/zhaoming-embedded/tree/master/oop-in-c/code/01-three-leds/)。
...
视频和书互相补强。视频更直观看口播节奏和挂号单类比的现场感。书里补了视频没讲透的 6 个细节（1.7 节）和 STM32 / Linux / 工业代码的对照（1.8 到 1.10 节）。
...
下一篇：[第 2 章 · 同事改了一行你的 LED 全乱了](02-同事改了一行.md)
