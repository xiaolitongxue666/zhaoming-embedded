...
这一节用的是函数式包装的 platform 抽象，是教学简化版。真正工业级用虚函数表（ops 表），允许 runtime 切换平台。第 11 章会演化对齐。
...
同样是函数式包装的教学简化版，第 11 章后演化为 ops 表。
...
`xxx_base` 是抽象基类（属于第 6 章继承的内容），`xxx_gpio` / `ds2433` / `max31827` 是具体实现。这层结构对应"虚函数表 + 具体设备的 ops 表"，是第 9 章到第 11 章的内容。
