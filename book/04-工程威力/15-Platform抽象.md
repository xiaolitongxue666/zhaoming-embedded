...
每个子类一份独立的 `.h / .c`，跟 `industrial/stm32_full/app/drivers/led/` 工业代码一字不差。教学上的好处：加一种新硬件 (例如 SPI 移位寄存器 LED) 只新增 `led_spi.h / led_spi.c` 一对文件，原有三个子类一字不动；review 时也能 `git log led_pwm.c` 直接拿到 PWM 这一路独立的演化历史。`led.h / led.c` 单文件版本 ch15 第一稿用过, 拆开后子类边界更清晰, ch16 / 附录 B / 附录 C 也都按这个组织.
...
ch15 主线只展示 `platform_pin` / `platform_pwm` / `platform_i2c` 三个外设。真实工业项目 (本书 `industrial/platform_layer/` 脱敏版, 脱胎自一台 STM32H7 控制板) 覆盖一组 12 个 platform 接口:

> 注: 表格列的是工业完整版 (`industrial/platform_layer/`) 的 ops 数·教学版 § 15.17.3 简化为只 `master_xfer`·让读者第一眼跑通。`slave_xfer / control` 在 industrial/ 实际工程里齐全·读者真机做完整工程参考 industrial/。

| 外设 | ops 函数指针数 | bus / device 二层? | 文件量级 |
| --- | --- | --- | --- |
| device | 6 (init / open / close / read / write / control) | 基类 | ~400 行 |
| pin | 7 | 单层 | ~230 行 |
...
ch15 教学版选 3 个讲透机制, 完整 12 个接口看 `industrial/platform_layer/` (1:1 工业版, 已脱敏)。机制都是同一个 -- ops 表 + register 注册 -- 学一个推所有, 多出来的那 9 个看清 ops 表上挂什么函数即可。
...
> **二层抽象本身和 OS 无关**: `industrial/platform_layer/` 的 `struct platform_i2c_bus_device` 里有 `osMutex` 字段, 解决"多线程多 client 抢同一 bus"的并发问题。教学版单线程跑, 去掉 mutex, **保留 bus + client + msg 三个 struct 即可** —— 二层是设计抽象, mutex 是 OS 落地, 两件事分开看。读者读教学版时把 mutex 想象不存在, 机制本身完全成立; 真上 RTOS / Linux 多线程时把 mutex 加回来, 调一行 osMutexAcquire / osMutexRelease 包住 master_xfer 即可, 三个 struct 字段不动。
...
/* oop-in-c/code/15-platform/platform/platform_i2c.h, 教学简化版,
   签名和 industrial/platform_layer/platform_i2c.h 工业版对齐 */
...
```c
/* 简化自 industrial/, 真实工程的 max31827_temp_read */
...
spi 也是 bus + device 二层 (一颗 MCU 1-4 条 SPI bus, 每 bus 挂多个 cs 片选下的设备)。机制和 i2c 同源 -- 共享 SCK / MOSI / MISO 线, 用片选区分挂在 bus 上的多颗芯片。完整接口看 `industrial/platform_layer/platform_spi.h`, 字段对照 Linux `struct spi_master / struct spi_device`。
