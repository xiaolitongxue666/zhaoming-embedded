...
应用层 `app.c` / 父类 `led_base.c` / 子类 `led.c` / 板级 `board_init.c` 一字不改。封装函数 `platform_gpio_write` 的签名一字不改。换的就是这一份 `led_stm32.c`。
...
├── led_base.h, led_base.c 父类 - led_base + led_base_init + 父类统一接口
├── led.h, led.c           子类 - led_gpio / led_pwm / led_i2c
...
struct led_ops;     /* 前向声明, 完整定义在 led.h */
...
/* PWM 子类 (三件套全填) + I2C 子类 (只填 on/off): 同套路, 见配套源码. */
```
...
TARGET  = demo
SRCS    = main.c app.c led.c led_base.c board_init.c ../../common/platform_pc.c
