...
打开第 12 章配套代码 `oop-in-c/code/12-upcasting/`，跟第 11 章 `oop-in-c/code/11-polymorphism/` 对一下，会发现几处字段和签名变了。这些变化在主线叙事里没特别说，这里集中说一下。本章 § 12.1 引的 `led_gpio_init(&gpio_led, "ERR", 10, true)` 和 § 12.2 画的 `struct led_gpio { base; pin; on_level; }`，都已经是下面这个 ch12 的形态。
...
不是所有 LED 都是高电平亮。GPIO 反向接法（共阳极接法）需要拉低电平才点亮。`on_level` 把这个硬件极性差异封装在子类里，应用层 `led_on(handle)` 不用知道具体是高还是低。

`led_gpio_init` 多了一个参数：
...
第四个参数对应新加的 `on_level` 字段。返回类型从 `int` 改成 `void`，因为子类 init 失败现在直接 panic（板级 init 失败应该立刻暴露，而不是让上层去判断错误码）。
