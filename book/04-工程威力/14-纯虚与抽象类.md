...
第一种做法：在父类的统一接口里加一行检查。`led.c` 实际写法：
...
更好的做法：把"安静跳过"的默认行为放到父类的统一接口里。`led.c` 实际写法：
...
如果你想体验"忘了填 on"的崩溃：把 `led_gpio_init` 里的 `led_base_init(&me->base, name, &gpio_ops);` 这一行注释掉，`me->base.ops` 就是没装上的状态，再编译跑，`led_on` 里的 assert 立刻报错。
