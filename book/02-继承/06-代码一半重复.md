...
那本《设计模式》（GoF）有句被反复引用的话："优先用组合而不是继承"。这句话是 90 年代为应用层 Java / C++ 业务系统说的：那种场景里多层继承容易让基类一改全崩。但内核驱动这种"统一接口 + 大量芯片实现"的场景，继承（把父类嵌进子类第一个字段）才是世界标准，解决的就是"N 家芯片 + 1 套接口"的问题，没有第二个答案。
