...
`const void *api` 就是 ch11 推过的"父类 vptr"。类型为什么擦除成 `void *`，原因是各子系统的 ops 表 struct 不一样，`led_driver_api`、`gpio_driver_api`、`i2c_driver_api` 都不是同一个 struct。父类层不知道每个子系统具体的 ops 表长什么样，只能擦成 `void *`，调用前由子系统头文件做向下转型。
...
`(const struct led_driver_api *)dev->api` 这一行，就是 ch11 推过的"父类 vptr 向下转型到子系统 ops 表"。父类层把 ops 表存成 `void *`，调用前每个子系统的公开头文件负责强转回自己的 ops 表 struct，然后通过 `api->on` 一次间接跳转，命中具体的子类实现。
...
第一行 `const struct led_gpio_config *config = dev->config;` 是 ch13 讲过的"子类向下转型 dev_data"。父类把出厂参数存在 `dev->config` 这个 `const void *` 字段里，子类一进来就把它强转回自己的 `led_gpio_config *`，然后访问字段。这就是 ch13 推过的 container 反推逻辑的轻量版，因为这里 config 直接就是 void * 转回去，不用 offsetof 减偏移。
...
- `led_gpio_set_brightness` 等实现函数·24 行：对应 ch12 子类实现 + ch13 `dev->config` 反推
...
| ch11 | 公开 dispatch + 向下转型 | `z_impl_led_on` 强转 `dev->api` | `include/zephyr/drivers/led.h` |
...
| ch13 | 子类向下转型 dev_data | `dev->config` 强转 `led_gpio_config *` | `drivers/led/led_gpio.c` |
