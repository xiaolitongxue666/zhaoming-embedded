...
第 10 章每颗 LED 自带 ops 表。应用层调 `led_on(&red_led.base)`，内部 `me->ops->on(me)` 走对的实现。

但应用层有个不爽的地方：每次都得自己写 `me->ops->on(me)` 这一长串。能不能直接调一个 `led_on(me)`，让它自己找到对的函数？
...
同一行代码 `led_on(...)`，跑出三种不同行为。它怎么知道该调哪个？
...
这一章 `led_on` 是简化版，只做 NULL 防御 + dispatch 两件事。后面章节会引入"必填 / 选填"策略和 `assert` 抓忘填的子类，那是给函数表加一份契约，本章主题是"同名函数不同行为"，先不展开。
...
`led_on / led_off / led_toggle` 这 3 个对外 API 一行不改。`struct led_base` 一行不改。`struct led_ops` 一行不改。所有应用层代码一行不改。

这就是函数指针表 + base + dispatch 这一套在工程上的好处：加新功能不动老代码，老代码不会被加新东西改坏。
...
STM32 端 `led_base.h / led_base.c / led.h / led.c / main.c` 一字不改。`platform_gpio_xxx` 这一组封装函数还是 ch01 那套：
...
## 11.9 你现在的代码在 Linux 用户态长什么样

Linux 端 sysfs 实现一字不改：
...
启动时跑一段 sysfs export，把要用的 GPIO 编号 export 出来；之后所有 LED 通过 `platform_gpio_xxx` 封装函数走到 sysfs。`led_base / led / main.c` 一字不改。

同 11.8 节，platform 层用的是函数式包装的教学简化版。第 16 章会演化成 ops 表的可切换形态，让一份 firmware 同时持有 PC / STM32 / Linux 三套 platform 实例运行时切换。
...
========================================
  ...
  Same call, different behavior per LED.
========================================
...
注意 ops 地址：红灯和绿灯都打到 `XXXXXXXX`（共享同一张 `led_ops_gpio` 表），蓝灯打到 `YYYYYYYY`（`led_ops_pwm`）。具体地址因 build 环境会变，关键是看红灯绿灯共享同一地址，蓝灯不同。同一个循环，三颗 LED 各自 dispatch 到对的实现。
