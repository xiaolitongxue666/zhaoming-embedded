...
PC 上跑通的 `test_led(on, off, id)` 函数本身一字不改。`gpio_on / gpio_off / pwm_on / pwm_off / i2c_on / i2c_off` 这 6 个函数的 signature 也跟 PC 版一字不变（都是 `void name(int param)`），只是函数体里改成调 STM32 HAL（节选自 [`oop-in-c/code/08-callback/platform-mcu/stm32/led_stm32.c`](https://github.com/ZhaoChengBo/zhaoming-embedded/tree/master/oop-in-c/code/08-callback/platform-mcu/stm32/led_stm32.c)，pin 仍是 `PIN_NUM('A', 13)` 编码，详见第 1 章 § 1.x PIN_NUM 编码）：
