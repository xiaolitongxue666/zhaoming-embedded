...
第 7 章你把"调谁开灯"塞进了 `struct led` 的 on_func 字段。`led_on(&red_led)` 内部走 `me->on_func(me)`，运行时决定。

但有一种情况它解决不了。

你写一个 `test_led` 工具函数，要做的事是：开 → 等 → 关，三步。这个函数应该不挑 LED：传谁的 LED 实例进来，它都能跑完三步。

按 ch07 的写法，`test_led(struct led *me)` 内部调 `led_on(me); led_off(me);`，确实和 LED 类型解耦。但你想加一个新需求：测试时不用 me->on_func 字段里那个常驻的实现，而是临时换一个调试版的 on（比如 on 之前先打一行 log）。

让 `test_led` 临时换 on，怎么换？

直觉一：mutate `me->on_func`，跑完测试再换回来。一行写错就把生产代码的 on 字段污染了，下一次正常调用 led_on 就乱套。

直觉二：让 `test_led` 接受一个额外的参数，告诉它"这次测试用这个 on，不用 me 字段里的"。
...
子类的 on_func 字段被 `test_led` 完全旁路了。生产代码的字段不动，测试代码用自己临时传进去的 on。
...
`test_led(&red_led, led_on_gpio_style, led_off)` 这一调：

- r0 = &red_led
- r1 = led_on_gpio_style 的地址
- r2 = led_off 的地址
