...
不同的是 Linux 上**没有 platform 抽象层**。GPIO 子类内部直接调 libgpiod（`gpiod_line_set_value`），背后是 Linux 内核完整的 gpio chardev / gpiolib（内核已经把 platform 抽象做完了，应用层再套一层是反工程）。完整工程见附录 C。
