 ...
 *
 * 教学变形：故意让 base 不在第 0 字段，演示 container_of 与位置无关。
 * 真实工业代码里 base 仍然推荐放在第一个位置。换 magic 在前
 * container_of 一字不改，这才是它的核心威力。
 *
...
/* GPIO 子类：base 故意不放第一个位置（教学变形，仅本章演示用）
 *
 ...
 *
 * 教学变形说明：真实工业代码里 base 仍推荐放第一个字段。本章故意挪
 * 一下，是为了让 container_of "与位置无关"这一性质能被肉眼看到。
 *
 * 详见 ch13 § 13.8.1 编译器算偏移的过程 + § 13.8.2 位置无关 demo。
 */
struct led_gpio {
