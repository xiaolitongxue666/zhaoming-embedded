 ...
 *
 * PWM 子类走 /sys/class/pwm/pwmchipN/pwmM, I2C 子类走 /dev/i2c-N
 * + ioctl(I2C_SLAVE) + write. 子类签名都是 int xxx(struct led_base *me),
 * 第一行 (struct led_xxx *)me 强转回子类 (ch12 还在用强转, container_of
 * 是 ch13 的演化).
 */
#include "led.h"
#include "platform.h"
 ...
 * duty_cycle / enable. 写入顺序: period 先于 duty_cycle, duty_cycle
 * <= period; enable 在 period / duty_cycle 都设好之后再写.
 */
...
{
	struct led_pwm *self = (struct led_pwm *)me;     /* ch12: 强转 */
 ...
 * 7-bit 从机地址绑到 fd, 后续 write 直接走 START + addr<<1 + data + STOP
 * 的标准 I2C 时序.
 *
 * 错误路径: open / ioctl 任意一步失败立即 close fd 返回 -1, 防止句柄
 * 泄漏. ioctl 返回 < 0 通常 errno = EBUSY (总线被占) 或 EREMOTEIO
 * (目标芯片没应答).
 *
 * cmd 字节就是写到芯片寄存器 0 的值: 0x01 表示开, 0x00 表示关. 真实芯
 * 片 (PCA9555 / TLC59116 等) 的寄存器映射要看 datasheet.
 */
