 ...
 *
 * @details
 * 见 ch12 § 12.4. 嵌入式工程里这种"模块 + 板级配置"文件叫板级初始化
 * (Board Support Package, BSP). 命名带模块前缀 led_board_init 而不是
 * 一个全局 board_init, 是因为真实工程一块板上不止 LED 一个外设, 还有
 * sensor / uart / motor 等等, 每个外设各自一份 xxx_board_init.c, 谁的
 * 硬件参数谁负责, 别全塞一个文件.
 *
 ...
 *     struct led_base * 类型, 在 led_board_init() 里被赋值 &xxx.base
 *
 * 换硬件方案 (例如报警灯 GPIO -> PWM) 只需改本文件三行 (子类类型、
 * init 调用、句柄绑定), main.c 一字不动. 见 ch12 § 12.6.
 *
 * led_board_init() 在 main 里开机调一次. 真实工程会有更长的初始化
 * 链 (时钟、中断向量、其他外设的 xxx_board_init、RTOS, 等等).
 */
...
/*
 * 全局句柄 - 真正的"定义". 绑定动作在 led_board_init 里完成,
...
int led_board_init(void)
{
	 ...
	 * base 字段, 再填子类自己的硬件资源.
	 *
	 * 三种 LED 三种硬件 (PC 版用占位参数, STM32 板级换真实硬件资源):
	 *   GPIO 灯 (ERR)   -> pin 10 占位, on_level = high
	 *   PWM  灯 (STAT)  -> channel = 1, duty = 50%
	 *   I2C  灯 (NET)   -> bus = 0, addr = 0x20
	 *
	 * 任意一颗 LED 初始化失败, led_board_init 直接把错误码返回给 main.
